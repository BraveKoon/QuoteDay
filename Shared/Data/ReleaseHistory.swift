...
public extension ReleaseHistory {
    ...
    static let all: [Release] = [
        Release(
            version: "1.8.9",
            date: "2026-09-24",
            summary: "홈 화면의 위젯과 앱이 서로 다른 명언을 보여 주던 것을 고쳤다.",
            sections: [
                ReleaseSection(
                    title: "고침",
                    items: [
                        "앱과 위젯이 같은 명언을 고른다. 두 곳이 각자 다른 규칙을 갖고 있었고, 규칙이 갈라져도 아무 데서도 오류가 나지 않아 조용히 어긋났다. 어긋난 곳이 둘이다. 하나, 카테고리를 어디서 읽는가 — 앱은 설정의 선호 카테고리를, 위젯은 자기만의 위젯 설정을 봤다. 둘, 카테고리와 원격 명언 중 무엇이 이기는가 — 앱은 원격이, 위젯은 카테고리가 이겼다. 정반대였다.",
                        "위젯이 앱 설정을 따른다. 위젯에서 따로 고르지 않았으면 앱에서 고른 카테고리를 쓴다. 위젯 설정의 \"✨ 전체\"는 \"✨ 앱 설정 따름\"으로 이름을 바꿨다 — 이 위젯만 다르게 보고 싶으면 카테고리를 직접 고르면 된다.",
                        "고른 카테고리가 원격 명언보다 우선한다. ZenQuotes 문장에는 카테고리가 없어서, 그대로 내보내면 고른 설정이 조용히 무시된다. 이 규칙은 이제 QuoteService.todayPresentation 한 곳에만 있다.",
                        "설정을 바꾸면 위젯이 곧바로 다시 그려진다. 선호 카테고리나 원격 명언 스위치를 바꿔도 위젯은 자정까지 옛 문장을 들고 있었다.",
                    ]
                ),
                ReleaseSection(
                    title: "더함",
                    items: [
                        "앱이 밟는 경로와 위젯이 밟는 경로의 결과를 모든 카테고리 × 원격 켬/끔 조합에서 맞춰 보는 테스트 9개. 자정을 넘길 때도 함께 넘어가는지 확인한다.",
                    ]
                ),
            ]
        ),
        ...
        Release(
            ...
            sections: [
                ReleaseSection(
                    title: "더함",
                    items: [
                        "랭킹 시즌. 랭킹이 석 달마다 리셋된다 — 1월·4월·7월·10월 1일에 모두가 0에서 다시 시작한다. 리셋이 없으면 먼저 시작한 사람이 계속 위에 있고 나중에 온 사람은 따라잡을 길이 없다.",
                        "결과 화면에 \"한 판 더\". 같은 단계를 이어서 하려고 화면을 닫고 목록으로 돌아갈 이유가 없다. 누르면 바로 새 판이 시작된다 — 문제는 매번 새로 뽑힌다.",
                    ]
                ),
                ReleaseSection(
                    title: "바뀜",
                    items: [
                        "20등 안에 들면 등수로, 그 아래는 상위 몇 %로 보여 준다. \"상위 3%\"보다 \"7등\"이 분명하고, 반대로 500등인 사람에게 \"500등\"은 막막하기만 하다. (전에는 인원이 20명보다 적을 때만 등수로 보여 줬다.)",
                    ]
                ),
                ReleaseSection(
                    title: "더함",
                    items: [
                        "시즌 경계(1·4·7·10월 1일), 시즌이 틈 없이 이어지는지, 리셋까지 남은 날을 확인하는 테스트.",
                        "시즌이 바뀌면 시즌 기록만 비워지고 통산 기록은 남는지 확인하는 테스트.",
                        "예전 버전에서 올라온 기록이 지워지지 않는지 확인하는 테스트.",
                        "같은 단계를 다시 시작하면 다른 문제가 나오는지 확인하는 테스트.",
                        "20등 경계에서 등수와 퍼센트 표시가 바뀌는지 확인하는 테스트.",
                    ]
                ),
            ]
        ),
        ...
        Release(
            ...
            sections: [
                ReleaseSection(
                    title: "고침",
                    items: [
                        "하트 수가 저절로 2 와 0 을 오가던 문제. 조회 실패를 \"레코드가 없다\"로 읽고 있었다. try? await database.record(for:) 는 둘을 한 값(nil)으로 만든다. 그래서 CloudKit 이 요청을 조이기 시작하면(Error rate mitigation activated) 실패한 조회가 \"아직 아무도 안 눌렀다\"가 되어 화면에 0 이 찍혔다. 없을 때(unknownItem)만 nil 로 보고 나머지 실패는 그대로 던진다. 던지면 화면은 마지막으로 알던 수를 그대로 둔다.",
                        "하트를 연타하면 전송이 겹치던 문제. 탭마다 전송이 하나씩 떠서 같은 레코드를 동시에 읽고 썼다. 한쪽은 record to insert already exists, 합계 쪽은 client oplock error updating record 가 났고 결국 요청이 조여졌다. 명언 하나당 전송은 하나만 돌고, 올리는 동안 또 누르면 그 전송이 끝나면서 마지막 상태를 이어서 올린다.",
                        "저장·삭제를 멱등하게. 이미 들어가 있는데 또 넣으려다 실패하면 원하는 상태는 이미 만들어진 것이므로 합계를 건드리지 않는다. 여기서 합계를 올리면 한 사람의 하트가 두 번 세어진다. 이미 지워진 것을 또 지우는 경우도 같다.",
                        "여러 명언의 하트 수를 읽을 때 레코드 안의 필드 대신 물어본 ID 로 키를 잡는다. 필드가 비어 있다고 그 명언의 수를 통째로 버리면 화면에서 0 으로 보인다.",
                        "올리는 사이에 또 누르면 서버가 준 수가 한 박자 늦은 값이다. 그때는 화면을 되돌리지 않고 새 값이 올라간 뒤에 맞춘다.",
                    ]
                ),
                ReleaseSection(
                    title: "더함",
                    items: [
                        "연타해도 같은 명언의 전송이 겹치지 않는지 확인하는 테스트.",
                        "조회가 실패해도 화면의 하트 수가 0 으로 떨어지지 않는지 확인하는 테스트.",
                    ]
                ),
            ]
        ),
        Release(
            ...
            sections: [
                ReleaseSection(
                    title: "고침",
                    items: [
                        "챌린지 점수가 CloudKit 의 시스템 Users 레코드에 섞여 들어가던 문제. 점수 레코드 이름으로 사용자 레코드 이름을 그대로 썼는데, 그 이름은 CloudKit 이 이미 쓰고 있는 Users 레코드의 이름이다. 그래서 ChallengePlayerScore 는 한 번도 만들어지지 않고 total 과 bucket 이 Users 레코드에 붙었다. 이름 앞에 score| 를 붙여 갈라 놓는다.",
                        "첫 제출이 0 구간이면 랭킹 분포가 올라가지 않던 문제. 위 문제 때문에 Users 레코드가 늘 잡혀서 \"이전 구간이 있다\"고 잘못 읽었고, 값이 없는 것과 0 구간을 구분하지 않아 분포를 고치지 않고 넘어갔다. 그래서 ChallengeRankBucket 이 대시보드에 나타나지 않았다.",
                    ]
                ),
                ReleaseSection(
                    title: "더함",
                    items: [
                        "레코드 이름이 시스템 Users 레코드와 겹치지 않는지 확인하는 테스트.",
                    ]
                ),
            ]
        ),
